...
import { WeeklyGoalsCompletionResultStructOutput } from '../typechain-types/contracts/FitnessUnbreakableVow';
...
describe("EndToEndTest", function () {
    ...
    describe("Tests that the contract is able to execute several calls and still be consistent", function () {
        it("Should process multiple random contract interactions", async function () {
            ...
            const completedGoalsHistory: WeeklyGoalsCompletionResultStructOutput[] = [];
...
async function assertVowRemaningFunds(
    owner: any,
    enforcerAddress: any,
    completedGoalsHistory: WeeklyGoalsCompletionResultStructOutput[],
...
async function assertCorrectPhysicalActivityRecords(
    ...
    await assertWhetherPenaltyShouldBeApplied(completedGoals, enforcerAddress, fitnessUnbreakableVow);
    await assertWeeklyGoalsCompletion(fitnessUnbreakableVow, weekNumber, completedGoals);

    return completedGoals;
}

async function assertWeeklyGoalsCompletion(fitnessUnbreakableVow: FitnessUnbreakableVow, weekNumber: number, completedGoals: WeeklyGoalsCompletionResultStructOutput) {
...
async function assertWhetherPenaltyShouldBeApplied(completedGoals: WeeklyGoalsCompletionResultStructOutput, enforcerAddress: any, fitnessUnbreakableVow: FitnessUnbreakableVow) {
    const goalWasCompleted = completedGoals[0] === 1n;

    if (goalWasCompleted) {
        await assertNoPenaltyWhenEnforceAgreement(fitnessUnbreakableVow);
    } else {
        await assertPenaltyWhenEnforceAgreement(await fitnessUnbreakableVow.PENALTY_AMOUNT(), enforcerAddress, fitnessUnbreakableVow);
    }
}
...
async function assertPenaltyWhenEnforceAgreement(penaltyAmount: bigint, enforcerAddress: any, fitnessUnbreakableVow: FitnessUnbreakableVow) {
    ...
    await expect(transaction).to.emit(fitnessUnbreakableVow, 'PenaltyApplied').withArgs(penaltyAmount, enforcerAddress.address);
}
...
function getCompletedGoals(physicalActivityRecordStruct: PhysicalActivityRecordStruct): WeeklyGoalsCompletionResultStructOutput {
    const wentoToTheGymEnoughTimes = BigInt(physicalActivityRecordStruct.gymVisits) >= 1n;
    const ran2km = BigInt(physicalActivityRecordStruct.runDistanceMeters) > 2000n;
    const sleptWell = BigInt(physicalActivityRecordStruct.healthySleepNights) >= 2;
    const isCompleted = (wentoToTheGymEnoughTimes && ran2km) || (wentoToTheGymEnoughTimes && sleptWell) || (ran2km && sleptWell);

    const output = [isCompleted ? 1n : 4n, wentoToTheGymEnoughTimes, ran2km, sleptWell] as WeeklyGoalsCompletionResultStructOutput;
