...
let PhysicalActivityOracle: Contract | null = null;
let FitnessUnbreakableVow: Contract | null = null;
...
export default async function loadContract() {
    ...
    PhysicalActivityOracle ||= await getContract('PhysicalActivityOracle', network, provider);
    FitnessUnbreakableVow ||= await getContract('FitnessUnbreakableVow', network, provider);

    const fromBlock = Number(addresses[`${network}.blockNumber`]);

    return {
        PhysicalActivityOracle,
        FitnessUnbreakableVow,
        FitnessUnbreakableVowAddress: addresses[`${network}.FitnessUnbreakableVow`],
        network,
        getEvents: <T>(contract: Contract, eventName: string, indexes: string[], data: string[]) => {
            return getEvents<T>(contract, eventName, indexes, fromBlock, data);
        },
...
async function getEvents<T>(contract: Contract, eventName: string, indexes: string[], fromBlockNumber: number, data: string[]) {
    const filter = contract.filters[eventName](...indexes)!;

    const events = (await contract.queryFilter(filter, fromBlockNumber) as EmittedEvent[]) || [];
