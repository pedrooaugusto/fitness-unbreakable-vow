import { BrowserProvider, ethers, JsonRpcSigner, type Eip1193Provider, type Eip6963ProviderInfo } from "ethers";
...
const getWalletCurrentNetwork = async (wallet: Wallet): Promise<Network> => {
    const chainId = parseInt(await wallet.provider.request({ method: 'eth_chainId' }), 16);

    return NetworkChainIdMap[chainId] as Network;
}
